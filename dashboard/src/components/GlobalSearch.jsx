  ...
  useEffect(() => {
    ...
    institutes.forEach((i) => {
      if (i.name?.toLowerCase().includes(q) || i.district?.toLowerCase().includes(q) || i.state?.toLowerCase().includes(q)) {
        found.push({ type: "institute", icon: "🏛️", label: i.name, sub: `${i.district}, ${i.state} — Risk: ${i.risk_score}`, id: i.id });
      }
    });
    inspections.forEach((i) => {
      if (i.institute_name?.toLowerCase().includes(q) || i.inspector_name?.toLowerCase().includes(q)) {
        found.push({ type: "inspection", icon: "🔍", label: `INS-${i.id} — ${i.institute_name}`, sub: `${i.status} • ${i.type}`, id: i.id });
      }
    });
    alerts.forEach((a) => {
      if (a.title?.toLowerCase().includes(q) || a.institute_name?.toLowerCase().includes(q)) {
        found.push({ type: "alert", icon: "🚨", label: a.title, sub: `${a.institute_name} — ${a.severity}`, id: a.id });
      }
    });
    complaints.forEach((c) => {
      if (c.description?.toLowerCase().includes(q) || c.institute_name?.toLowerCase().includes(q)) {
        found.push({ type: "complaint", icon: "📝", label: c.description?.slice(0, 60), sub: `${c.institute_name} — ${c.category}`, id: c.id });
      }
    });
    ...
    <div className="global-search" ref={ref}>
      <div className="search-input-wrap">
        ...
        <input
          ...
          placeholder="Search institutes, inspections, alerts, complaints..."

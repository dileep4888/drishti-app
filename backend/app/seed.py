"""Seed the database with demo data for DRISHTI AI."""
import random
...
def seed():
    ...
    for name, typ, scheme, state, dist, lat, lng, risk, rlevel, trust, ben, staff in institute_data:
        inst = Institute(
            ...
            contact_person=f"Contact - {name[:15]}", contact_phone=f"+91-{random.randint(70000,99999)}{random.randint(10000,99999)}",
            status="active",
        )
